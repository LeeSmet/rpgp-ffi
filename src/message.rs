...
/// Create a new message from a file name and data.
#[no_mangle]
pub unsafe extern "C" fn rpgp_msg_new_raw(
    file_name_ptr: *const libc::c_char,
    data_ptr: *const u8,
    data_len: libc::size_t,
) -> *mut Message {
    assert!(!file_name_ptr.is_null());
    assert!(!data_ptr.is_null());

    let file_name = CStr::from_ptr(file_name_ptr);
    let file_name_str = try_ffi!(file_name.to_str(), "invalid file name");
    let data_slice = slice::from_raw_parts(data_ptr, data_len);

    Box::into_raw(Box::new(Message::new_literal_bytes(
        file_name_str,
        data_slice,
    )))
}
...
#[no_mangle]
pub unsafe extern "C" fn rpgp_msg_to_str(msg_ptr: *const message) -> *mut libc::c_char {
    assert!(!msg_ptr.is_null());

    let msg = &*msg_ptr;

    let result = try_ffi!(msg.get_content(), "failed to get message content").unwrap(); // TODO: will fail on encrypted messages
